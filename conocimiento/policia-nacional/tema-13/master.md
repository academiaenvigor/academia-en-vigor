# TEMA 13 · SEGURIDAD PRIVADA: ORGANIZACIÓN, PERSONAL, SERVICIOS Y MEDIDAS

<!-- content_version: 1.0.0 -->
<!-- opposition: policia-nacional -->
<!-- status: approved_internal; publication: not_published -->

> Alcance editorial: epígrafe oficial de Escala Básica publicado el 10 de julio de 2026. Corte normativo: 2026-07-30. Redacción propia reconstruida desde fuentes oficiales. Los documentos privados se emplearon exclusivamente como inventario de cobertura.

<!-- BLOCK 01 START -->
## 1. Objeto, modelo y subordinación
**Fuente principal:** `CONV-PN-2026,L5-2014-T13`
<!-- PARTE START -->
La seguridad privada protege personas y bienes mediante actividades contratadas y sometidas a control público.

- La Ley 5/2014 regula actividades, servicios, personal, empresas, medidas e investigación privada.
- Las actividades de seguridad privada son complementarias y subordinadas respecto de la seguridad pública.
- La seguridad privada puede ser contratada voluntariamente o venir impuesta legalmente.

:::visual
**Referencia visual prevista:** `t13-02-modelo-complementario.webp` · Relación jerárquica entre seguridad pública, control y prestación privada.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La seguridad privada protege personas y bienes mediante actividades contratadas y sometidas a control público.

La ley la configura como complementaria y subordinada respecto de la seguridad pública. La contratación privada no crea una policía paralela ni desplaza las competencias de las Fuerzas y Cuerpos de Seguridad.

### Claves normativas

- La Ley 5/2014 regula actividades, servicios, personal, empresas, medidas e investigación privada.
- Las actividades de seguridad privada son complementarias y subordinadas respecto de la seguridad pública.
- La seguridad privada puede ser contratada voluntariamente o venir impuesta legalmente.

<!-- FACT:PN-T13-F001 -->
<!-- FACT:PN-T13-F002 -->
<!-- FACT:PN-T13-F003 -->

:::visual
**Referencia visual prevista:** `t13-02-modelo-complementario.webp` · Relación jerárquica entre seguridad pública, control y prestación privada.
:::

:::hablemos-claro
Privada en la contratación; pública en el marco y el control.
:::

:::en-la-calle
Un vigilante protege el servicio contratado, pero debe colaborar cuando aparecen hechos que afectan a la seguridad pública.
:::

:::lo-que-cae
Complementaria no significa independiente.
:::
<!-- ATESTADO END -->
<!-- BLOCK 01 END -->

<!-- BLOCK 02 START -->
## 2. Definiciones que no deben mezclarse
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Actividad, servicio, función y medida describen planos distintos de la seguridad privada.

- Las actividades son los ámbitos materiales en que actúan los prestadores.
- Los servicios son las acciones realizadas para materializar las actividades.
- Las funciones son las facultades atribuidas al personal de seguridad privada.

:::visual
**Referencia visual prevista:** `t13-03-cuatro-conceptos.webp` · Matriz de actividad, servicio, función y medida con ejemplos.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Actividad, servicio, función y medida describen planos distintos de la seguridad privada.

La actividad es el ámbito empresarial; el servicio es la acción contratada; la función es la facultad del personal; la medida es la disposición de prevención o protección.

### Claves normativas

- Las actividades son los ámbitos materiales en que actúan los prestadores.
- Los servicios son las acciones realizadas para materializar las actividades.
- Las funciones son las facultades atribuidas al personal de seguridad privada.

<!-- FACT:PN-T13-F004 -->
<!-- FACT:PN-T13-F005 -->
<!-- FACT:PN-T13-F006 -->

:::visual
**Referencia visual prevista:** `t13-03-cuatro-conceptos.webp` · Matriz de actividad, servicio, función y medida con ejemplos.
:::

:::hablemos-claro
Actividad contiene servicios; el personal ejerce funciones; las medidas protegen.
:::

:::en-la-calle
En una pregunta, cambia una sola palabra y cambia el sujeto jurídico.
:::

:::lo-que-cae
La lista de definiciones es terreno clásico de distractores.
:::
<!-- ATESTADO END -->
<!-- BLOCK 02 END -->

<!-- BLOCK 03 START -->
## 3. Ámbito subjetivo de aplicación
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La ley alcanza a prestadores, personal, servicios, medidas, usuarios y establecimientos obligados.

- La ley se aplica a empresas de seguridad, personal, despachos de detectives, servicios, medidas y contratos del sector.
- El régimen de inspección y sanción alcanza también a quien actúa sin autorización o habilitación.
- Los usuarios y establecimientos obligados pueden quedar sujetos a las disposiciones que les resulten pertinentes.

:::visual
**Referencia visual prevista:** `t13-04-ecosistema-sujetos.webp` · Ecosistema completo de sujetos incluidos en la ley.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La ley alcanza a prestadores, personal, servicios, medidas, usuarios y establecimientos obligados.

También se proyecta, cuando resulte pertinente, sobre técnicos, operadores, profesores, empresas de seguridad informática, centrales de uso propio y centros de formación.

### Claves normativas

- La ley se aplica a empresas de seguridad, personal, despachos de detectives, servicios, medidas y contratos del sector.
- El régimen de inspección y sanción alcanza también a quien actúa sin autorización o habilitación.
- Los usuarios y establecimientos obligados pueden quedar sujetos a las disposiciones que les resulten pertinentes.

<!-- FACT:PN-T13-F007 -->
<!-- FACT:PN-T13-F008 -->
<!-- FACT:PN-T13-F009 -->

:::visual
**Referencia visual prevista:** `t13-04-ecosistema-sujetos.webp` · Ecosistema completo de sujetos incluidos en la ley.
:::

:::hablemos-claro
La ley controla tanto a quien presta como a quien debe adoptar o contrata determinadas medidas.
:::

:::en-la-calle
En una inspección identifica prestador, personal, usuario, establecimiento y servicio.
:::

:::lo-que-cae
Ilegal no significa invisible para la ley.
:::
<!-- ATESTADO END -->
<!-- BLOCK 03 END -->

<!-- BLOCK 04 START -->
## 4. Fines de la seguridad privada
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Los fines combinan necesidades legítimas del usuario, contribución a la seguridad pública e integración funcional.

- Un fin es satisfacer necesidades legítimas de seguridad o información del usuario.
- La seguridad privada contribuye a prevenir infracciones y aportar información a procedimientos.
- La seguridad privada integra funcionalmente medios y capacidades como recurso externo de la seguridad pública.

:::visual
**Referencia visual prevista:** `t13-05-tres-fines.webp` · Tres engranajes de protección, contribución e integración.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Los fines combinan necesidades legítimas del usuario, contribución a la seguridad pública e integración funcional.

La actividad protege indemnidad, privacidad y bienes; previene infracciones y aporta información; y complementa el monopolio estatal de la seguridad sin apropiárselo.

### Claves normativas

- Un fin es satisfacer necesidades legítimas de seguridad o información del usuario.
- La seguridad privada contribuye a prevenir infracciones y aportar información a procedimientos.
- La seguridad privada integra funcionalmente medios y capacidades como recurso externo de la seguridad pública.

<!-- FACT:PN-T13-F010 -->
<!-- FACT:PN-T13-F011 -->
<!-- FACT:PN-T13-F012 -->

:::visual
**Referencia visual prevista:** `t13-05-tres-fines.webp` · Tres engranajes de protección, contribución e integración.
:::

:::hablemos-claro
Tres fines: proteger, contribuir e integrar.
:::

:::en-la-calle
La legitimidad de la necesidad contratada es el primer filtro.
:::

:::lo-que-cae
No confundas cooperación con condición funcionarial.
:::
<!-- ATESTADO END -->
<!-- BLOCK 04 END -->

<!-- BLOCK 05 START -->
## 5. Principios rectores y colaboración
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La prestación debe respetar Constitución, ley, principios profesionales y colaboración con las Fuerzas y Cuerpos de Seguridad.

- Los prestadores deben colaborar en todo momento y lugar con las Fuerzas y Cuerpos de Seguridad.
- Deben seguir las instrucciones policiales relacionadas con servicios que afecten a la seguridad pública o a sus competencias.
- La prestación debe respetar la Constitución y el resto del ordenamiento jurídico.

:::visual
**Ilustración simple:** `t13-il-01-relevo-coordinado.webp` · Vigilante y policía coordinan una intervención sin confundirse de función.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La prestación debe respetar Constitución, ley, principios profesionales y colaboración con las Fuerzas y Cuerpos de Seguridad.

La obligación de auxilio, colaboración y seguimiento de instrucciones se intensifica cuando el servicio afecta a seguridad pública o competencias policiales.

### Claves normativas

- Los prestadores deben colaborar en todo momento y lugar con las Fuerzas y Cuerpos de Seguridad.
- Deben seguir las instrucciones policiales relacionadas con servicios que afecten a la seguridad pública o a sus competencias.
- La prestación debe respetar la Constitución y el resto del ordenamiento jurídico.

<!-- FACT:PN-T13-F013 -->
<!-- FACT:PN-T13-F014 -->
<!-- FACT:PN-T13-F015 -->

:::visual
**Ilustración simple:** `t13-il-01-relevo-coordinado.webp` · Vigilante y policía coordinan una intervención sin confundirse de función.
:::

:::hablemos-claro
Contrato privado, límites públicos.
:::

:::en-la-calle
Ante un delito, preservar, comunicar y seguir instrucciones pesa más que la comodidad del servicio.
:::

:::lo-que-cae
Colaborar no autoriza a invadir competencias.
:::
<!-- ATESTADO END -->
<!-- BLOCK 05 END -->

<!-- BLOCK 06 START -->
## 6. Prohibiciones y reserva de información
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La seguridad privada no puede controlar ideas, interferir conflictos ni comunicar información protegida fuera de los cauces legales.

- No puede ejercerse control sobre opiniones políticas, sindicales o religiosas.
- La información conocida por razón del servicio está sometida a reserva, salvo comunicación legítima a autoridades judiciales o policiales.
- Está prohibido ejercer funciones de seguridad privada sin habilitación o acreditación exigible.

:::visual
**Referencia visual prevista:** `t13-07-semáforo-limites.webp` · Semáforo de conductas permitidas, condicionadas y prohibidas.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La seguridad privada no puede controlar ideas, interferir conflictos ni comunicar información protegida fuera de los cauces legales.

Tampoco pueden prestarse o publicitarse servicios sin autorización, ejercerse funciones sin habilitación ni usar medios que lesionen honor, intimidad, imagen o secreto de las comunicaciones.

### Claves normativas

- No puede ejercerse control sobre opiniones políticas, sindicales o religiosas.
- La información conocida por razón del servicio está sometida a reserva, salvo comunicación legítima a autoridades judiciales o policiales.
- Está prohibido ejercer funciones de seguridad privada sin habilitación o acreditación exigible.

<!-- FACT:PN-T13-F016 -->
<!-- FACT:PN-T13-F017 -->
<!-- FACT:PN-T13-F018 -->

:::visual
**Referencia visual prevista:** `t13-07-semáforo-limites.webp` · Semáforo de conductas permitidas, condicionadas y prohibidas.
:::

:::hablemos-claro
Proteger no permite vigilar ideas ni comerciar con secretos.
:::

:::en-la-calle
La comunicación a Policía de un hecho delictivo no es una filtración prohibida.
:::

:::lo-que-cae
Reserva profesional y encubrimiento no son lo mismo.
:::
<!-- ATESTADO END -->
<!-- BLOCK 06 END -->

<!-- BLOCK 07 START -->
## 7. Competencias de la Administración del Estado
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El Estado ejerce autorización, inspección, control, sanción y coordinación en el ámbito que le corresponde.

- Corresponde al Estado autorizar, inspeccionar y sancionar en los términos de la Ley 5/2014.
- La Policía ejerce funciones de control sobre empresas, personal y servicios en su ámbito competencial.
- La Guardia Civil ejerce competencias específicas, entre otras, respecto de guardas rurales y sus especialidades.

:::visual
**Referencia visual prevista:** `t13-08-mapa-competencias.webp` · Mapa funcional de Interior, Policía, Guardia Civil y órganos autonómicos.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El Estado ejerce autorización, inspección, control, sanción y coordinación en el ámbito que le corresponde.

El Ministerio del Interior, la Dirección General de la Policía y la Dirección General de la Guardia Civil distribuyen competencias según materia, sujetos y territorio.

### Claves normativas

- Corresponde al Estado autorizar, inspeccionar y sancionar en los términos de la Ley 5/2014.
- La Policía ejerce funciones de control sobre empresas, personal y servicios en su ámbito competencial.
- La Guardia Civil ejerce competencias específicas, entre otras, respecto de guardas rurales y sus especialidades.

<!-- FACT:PN-T13-F019 -->
<!-- FACT:PN-T13-F020 -->
<!-- FACT:PN-T13-F021 -->

:::visual
**Referencia visual prevista:** `t13-08-mapa-competencias.webp` · Mapa funcional de Interior, Policía, Guardia Civil y órganos autonómicos.
:::

:::hablemos-claro
No memorices un órgano aislado: enlaza materia, territorio y sujeto.
:::

:::en-la-calle
Una incidencia con guardas rurales no sigue exactamente el mismo circuito que una empresa urbana.
:::

:::lo-que-cae
Autorizar, controlar y sancionar son verbos diferentes.
:::
<!-- ATESTADO END -->
<!-- BLOCK 07 END -->

<!-- BLOCK 08 START -->
## 8. Competencias autonómicas y registros
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Las comunidades autónomas con competencias asumidas ejercen funciones sobre actividades y servicios desarrollados en su territorio.

- Las comunidades autónomas pueden ejercer competencias asumidas estatutariamente sobre seguridad privada.
- Existe un Registro Nacional de Seguridad Privada y pueden existir registros autonómicos.
- La coordinación registral evita duplicidades y facilita el control administrativo.
<!-- PARTE END -->
<!-- ATESTADO START -->
Las comunidades autónomas con competencias asumidas ejercen funciones sobre actividades y servicios desarrollados en su territorio.

El Registro Nacional y los registros autonómicos aseguran constancia y coordinación. La inscripción no borra la distribución material de competencias.

### Claves normativas

- Las comunidades autónomas pueden ejercer competencias asumidas estatutariamente sobre seguridad privada.
- Existe un Registro Nacional de Seguridad Privada y pueden existir registros autonómicos.
- La coordinación registral evita duplicidades y facilita el control administrativo.

<!-- FACT:PN-T13-F022 -->
<!-- FACT:PN-T13-F023 -->
<!-- FACT:PN-T13-F024 -->

:::hablemos-claro
Competencia autonómica sí; sistema desconectado, no.
:::

:::en-la-calle
Comprueba dónde se presta el servicio y qué competencia ha asumido la comunidad.
:::

:::lo-que-cae
Registro no equivale a licencia universal.
:::
<!-- ATESTADO END -->
<!-- BLOCK 08 END -->

<!-- BLOCK 09 START -->
## 9. Acceso a información y coordinación
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La coordinación se apoya en información recíproca, órganos de participación y medidas organizativas.

- Las Fuerzas y Cuerpos de Seguridad pueden acceder a información necesaria para el ejercicio de sus funciones.
- El Ministerio del Interior o el órgano autonómico competente adopta medidas organizativas de coordinación.
- Los órganos de participación permiten intercambio entre sector y Administraciones sin sustituir la autoridad pública.

:::visual
**Referencia visual prevista:** `t13-10-centro-coordinacion.webp` · Centro de coordinación con flujos de información delimitados.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La coordinación se apoya en información recíproca, órganos de participación y medidas organizativas.

Las empresas y despachos deben facilitar información relevante para la seguridad pública, respetando límites legales. Interior o el órgano autonómico competente establece medidas de coordinación.

### Claves normativas

- Las Fuerzas y Cuerpos de Seguridad pueden acceder a información necesaria para el ejercicio de sus funciones.
- El Ministerio del Interior o el órgano autonómico competente adopta medidas organizativas de coordinación.
- Los órganos de participación permiten intercambio entre sector y Administraciones sin sustituir la autoridad pública.

<!-- FACT:PN-T13-F025 -->
<!-- FACT:PN-T13-F026 -->
<!-- FACT:PN-T13-F027 -->

:::visual
**Referencia visual prevista:** `t13-10-centro-coordinacion.webp` · Centro de coordinación con flujos de información delimitados.
:::

:::hablemos-claro
Información útil, cauce legal y autoridad competente.
:::

:::en-la-calle
Una central receptora comunica una alarma real con datos operativos, no todo el historial comercial del cliente.
:::

:::lo-que-cae
Coordinar no es ceder el mando público.
:::
<!-- ATESTADO END -->
<!-- BLOCK 09 END -->

<!-- BLOCK 10 START -->
## 10. Actividades reservadas, compatibles y excluidas
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La ley separa actividades de seguridad privada, actividades compatibles y actividades excluidas.

- La vigilancia y protección de bienes, lugares, eventos y personas determinadas es actividad de seguridad privada.
- La planificación y asesoramiento en seguridad pueden ser actividades compatibles en los términos legales.
- Las tareas de recepción, información o control de accesos sin funciones de seguridad pueden ser actividades excluidas.

:::visual
**Referencia visual prevista:** `t13-11-tres-zonas-actividad.webp` · Tres zonas: reservadas, compatibles y excluidas con ejemplos.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La ley separa actividades de seguridad privada, actividades compatibles y actividades excluidas.

Vigilancia, protección, depósito, transporte, instalación de sistemas conectados, gestión de alarmas e investigación privada son actividades reguladas. Otras tareas auxiliares no adquieren por ello naturaleza de seguridad.

### Claves normativas

- La vigilancia y protección de bienes, lugares, eventos y personas determinadas es actividad de seguridad privada.
- La planificación y asesoramiento en seguridad pueden ser actividades compatibles en los términos legales.
- Las tareas de recepción, información o control de accesos sin funciones de seguridad pueden ser actividades excluidas.

<!-- FACT:PN-T13-F028 -->
<!-- FACT:PN-T13-F029 -->
<!-- FACT:PN-T13-F030 -->

:::visual
**Referencia visual prevista:** `t13-11-tres-zonas-actividad.webp` · Tres zonas: reservadas, compatibles y excluidas con ejemplos.
:::

:::hablemos-claro
No preguntes solo qué tarea se hace; mira con qué facultades y finalidad.
:::

:::en-la-calle
Controlar una acreditación de acceso no autoriza a cachear o detener como vigilante.
:::

:::lo-que-cae
Auxiliar no es vigilante barato.
:::
<!-- ATESTADO END -->
<!-- BLOCK 10 END -->

<!-- BLOCK 11 START -->
## 11. Autorización y requisitos de empresas
**Fuente principal:** `L5-2014-T13,OINT314-2011-T13`
<!-- PARTE START -->
Las empresas necesitan autorización o declaración responsable cuando la ley la admite, inscripción y requisitos adecuados a la actividad.

- La prestación de actividades reservadas exige autorización administrativa o declaración responsable en los casos previstos.
- La empresa debe estar inscrita en el registro correspondiente.
- Los requisitos y garantías se ajustan a las actividades para las que se solicita autorización.
<!-- PARTE END -->
<!-- ATESTADO START -->
Las empresas necesitan autorización o declaración responsable cuando la ley la admite, inscripción y requisitos adecuados a la actividad.

La forma jurídica, objeto, medios, garantías y seguridad de instalaciones varían según actividades. La autorización no habilita para realizar cualquier actividad no declarada.

### Claves normativas

- La prestación de actividades reservadas exige autorización administrativa o declaración responsable en los casos previstos.
- La empresa debe estar inscrita en el registro correspondiente.
- Los requisitos y garantías se ajustan a las actividades para las que se solicita autorización.

<!-- FACT:PN-T13-F031 -->
<!-- FACT:PN-T13-F032 -->
<!-- FACT:PN-T13-F033 -->

:::hablemos-claro
Autorizar una empresa no es darle una llave maestra.
:::

:::en-la-calle
Comprueba actividad autorizada, ámbito, inscripción y medios antes de contratar.
:::

:::lo-que-cae
Objeto social y autorización administrativa no son equivalentes.
:::
<!-- ATESTADO END -->
<!-- BLOCK 11 END -->

<!-- BLOCK 12 START -->
## 12. Obligaciones y representantes de empresas
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La empresa debe mantener requisitos, comunicar cambios, asegurar formación y garantizar correcta prestación.

- La empresa debe comunicar las modificaciones relevantes que afecten a su autorización o inscripción.
- Debe garantizar que el personal asignado esté habilitado y formado para sus funciones.
- La empresa debe facilitar inspecciones y conservar la documentación exigida.

:::visual
**Ilustración simple:** `t13-il-02-sala-operaciones.webp` · Equipo de empresa revisa personal, contrato y medios antes del servicio.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La empresa debe mantener requisitos, comunicar cambios, asegurar formación y garantizar correcta prestación.

Los representantes legales deben reunir condiciones legales y responden de funciones organizativas. La empresa conserva documentación y facilita inspección.

### Claves normativas

- La empresa debe comunicar las modificaciones relevantes que afecten a su autorización o inscripción.
- Debe garantizar que el personal asignado esté habilitado y formado para sus funciones.
- La empresa debe facilitar inspecciones y conservar la documentación exigida.

<!-- FACT:PN-T13-F034 -->
<!-- FACT:PN-T13-F035 -->
<!-- FACT:PN-T13-F036 -->

:::visual
**Ilustración simple:** `t13-il-02-sala-operaciones.webp` · Equipo de empresa revisa personal, contrato y medios antes del servicio.
:::

:::hablemos-claro
La autorización se mantiene cumpliendo, no se archiva y se olvida.
:::

:::en-la-calle
Una auditoría de servicio empieza por contrato, comunicación, personal y medios.
:::

:::lo-que-cae
La empresa responde de organizar legalmente la prestación.
:::
<!-- ATESTADO END -->
<!-- BLOCK 12 END -->

<!-- BLOCK 13 START -->
## 13. Despachos de detectives privados
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La investigación privada se presta desde despachos habilitados y está separada de las actividades propias de empresas de seguridad.

- Los detectives privados prestan servicios de investigación desde despachos inscritos.
- El encargo debe responder a un interés legítimo del contratante.
- Las empresas de seguridad no pueden realizar la investigación privada propia de detectives, ni estos servicios propios de aquellas.

:::visual
**Referencia visual prevista:** `t13-14-frontera-investigacion.webp` · Frontera entre vigilancia empresarial e investigación de detectives.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La investigación privada se presta desde despachos habilitados y está separada de las actividades propias de empresas de seguridad.

El despacho exige apertura, inscripción y obligaciones documentales. Solo puede aceptar encargos con interés legítimo y dentro de límites legales.

### Claves normativas

- Los detectives privados prestan servicios de investigación desde despachos inscritos.
- El encargo debe responder a un interés legítimo del contratante.
- Las empresas de seguridad no pueden realizar la investigación privada propia de detectives, ni estos servicios propios de aquellas.

<!-- FACT:PN-T13-F037 -->
<!-- FACT:PN-T13-F038 -->
<!-- FACT:PN-T13-F039 -->

:::visual
**Referencia visual prevista:** `t13-14-frontera-investigacion.webp` · Frontera entre vigilancia empresarial e investigación de detectives.
:::

:::hablemos-claro
Encargo legítimo, objeto lícito y despacho inscrito.
:::

:::en-la-calle
Antes de aceptar un caso, el detective debe poder explicar qué derecho o interés legítimo se protege.
:::

:::lo-que-cae
Investigar no autoriza a invadir intimidad.
:::
<!-- ATESTADO END -->
<!-- BLOCK 13 END -->

<!-- BLOCK 14 START -->
## 14. Categorías de personal habilitado
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La ley enumera categorías profesionales con funciones propias y no intercambiables.

- El vigilante de explosivos es una especialidad del vigilante de seguridad.
- Guardas de caza y guardapescas marítimos son especialidades de los guardas rurales.
- Jefes y directores de seguridad son categorías distintas con funciones diferenciadas.

:::visual
**Referencia visual prevista:** `t13-15-arbol-profesional.webp` · Árbol de categorías y especialidades del personal.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La ley enumera categorías profesionales con funciones propias y no intercambiables.

Comprende vigilantes y su especialidad de explosivos, escoltas, guardas rurales y sus especialidades, jefes y directores de seguridad y detectives privados.

### Claves normativas

- El vigilante de explosivos es una especialidad del vigilante de seguridad.
- Guardas de caza y guardapescas marítimos son especialidades de los guardas rurales.
- Jefes y directores de seguridad son categorías distintas con funciones diferenciadas.

<!-- FACT:PN-T13-F040 -->
<!-- FACT:PN-T13-F041 -->
<!-- FACT:PN-T13-F042 -->

:::visual
**Referencia visual prevista:** `t13-15-arbol-profesional.webp` · Árbol de categorías y especialidades del personal.
:::

:::hablemos-claro
Categoría, especialidad y función: tres columnas que deben encajar.
:::

:::en-la-calle
El uniforme no identifica por sí solo la habilitación concreta.
:::

:::lo-que-cae
Vigilante de explosivos sí necesita la habilitación previa de vigilante.
:::
<!-- ATESTADO END -->
<!-- BLOCK 14 END -->

<!-- BLOCK 15 START -->
## 15. Habilitación y requisitos generales
**Fuente principal:** `L5-2014-T13,OINT318-2011-T13`
<!-- PARTE START -->
El ejercicio de funciones exige habilitación profesional y mantenimiento de requisitos de aptitud y honorabilidad.

- La habilitación profesional es previa al ejercicio de las funciones reservadas.
- No haber sido sancionado por infracción grave o muy grave en los períodos legales es un requisito general.
- La formación y las pruebas acreditan conocimientos y aptitudes para la categoría.
<!-- PARTE END -->
<!-- ATESTADO START -->
El ejercicio de funciones exige habilitación profesional y mantenimiento de requisitos de aptitud y honorabilidad.

La ley contempla edad, capacidad, formación, aptitud física y psíquica, carencia de determinados antecedentes y sanciones, además de requisitos de nacionalidad o régimen equiparado.

### Claves normativas

- La habilitación profesional es previa al ejercicio de las funciones reservadas.
- No haber sido sancionado por infracción grave o muy grave en los períodos legales es un requisito general.
- La formación y las pruebas acreditan conocimientos y aptitudes para la categoría.

<!-- FACT:PN-T13-F043 -->
<!-- FACT:PN-T13-F044 -->
<!-- FACT:PN-T13-F045 -->

:::hablemos-claro
Habilitación antes, función después.
:::

:::en-la-calle
Verifica categoría y vigencia de la tarjeta de identidad profesional antes de asignar el puesto.
:::

:::lo-que-cae
Contratar no habilita.
:::
<!-- ATESTADO END -->
<!-- BLOCK 15 END -->

<!-- BLOCK 16 START -->
## 16. Principios de actuación y protección jurídica
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El personal actúa con legalidad, integridad, dignidad, corrección, congruencia y proporcionalidad.

- Legalidad, integridad, dignidad, corrección, congruencia y proporcionalidad son principios de actuación.
- La fuerza solo puede emplearse cuando sea necesaria y de forma proporcional.
- La protección jurídica reforzada se vincula a personal identificado que coopera y actúa bajo mando policial.

:::visual
**Ilustración simple:** `t13-il-03-escudo-proporcional.webp` · Vigilante protegido por un escudo con principios de actuación.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El personal actúa con legalidad, integridad, dignidad, corrección, congruencia y proporcionalidad.

Cuando coopera y actúa bajo mando de las Fuerzas y Cuerpos de Seguridad, el ordenamiento le reconoce protección jurídica reforzada frente a agresiones en los términos legales.

### Claves normativas

- Legalidad, integridad, dignidad, corrección, congruencia y proporcionalidad son principios de actuación.
- La fuerza solo puede emplearse cuando sea necesaria y de forma proporcional.
- La protección jurídica reforzada se vincula a personal identificado que coopera y actúa bajo mando policial.

<!-- FACT:PN-T13-F046 -->
<!-- FACT:PN-T13-F047 -->
<!-- FACT:PN-T13-F048 -->

:::visual
**Ilustración simple:** `t13-il-03-escudo-proporcional.webp` · Vigilante protegido por un escudo con principios de actuación.
:::

:::hablemos-claro
Poder limitado por principios claros.
:::

:::en-la-calle
La actuación más eficaz es la que resuelve con la mínima intervención necesaria.
:::

:::lo-que-cae
Protección de agente no convierte al vigilante en policía.
:::
<!-- ATESTADO END -->
<!-- BLOCK 16 END -->

<!-- BLOCK 17 START -->
## 17. Vigilantes de seguridad
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Los vigilantes protegen bienes, establecimientos, eventos y personas dentro del servicio contratado.

- Los vigilantes ejercen vigilancia y protección de bienes, lugares, eventos y personas.
- Pueden realizar controles de identidad en el acceso o interior sin retener documentación personal.
- Ante un delito relacionado con el objeto protegido, deben detener y poner inmediatamente a disposición de las Fuerzas y Cuerpos de Seguridad al autor y efectos.

:::visual
**Referencia visual prevista:** `t13-18-ciclo-vigilante.webp` · Ciclo de prevención, control, intervención y entrega policial.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Los vigilantes protegen bienes, establecimientos, eventos y personas dentro del servicio contratado.

Pueden efectuar controles de identidad y objetos en el acceso o interior, impedir acceso o permanencia y detener y poner inmediatamente a disposición policial a quien sorprendan en relación con el objeto protegido.

### Claves normativas

- Los vigilantes ejercen vigilancia y protección de bienes, lugares, eventos y personas.
- Pueden realizar controles de identidad en el acceso o interior sin retener documentación personal.
- Ante un delito relacionado con el objeto protegido, deben detener y poner inmediatamente a disposición de las Fuerzas y Cuerpos de Seguridad al autor y efectos.

<!-- FACT:PN-T13-F049 -->
<!-- FACT:PN-T13-F050 -->
<!-- FACT:PN-T13-F051 -->

:::visual
**Referencia visual prevista:** `t13-18-ciclo-vigilante.webp` · Ciclo de prevención, control, intervención y entrega policial.
:::

:::hablemos-claro
Controlar sí; retener documentos, no.
:::

:::en-la-calle
Tras la detención, seguridad, aviso y entrega inmediata: el servicio no es una comisaría.
:::

:::lo-que-cae
Detener no autoriza a interrogar.
:::
<!-- ATESTADO END -->
<!-- BLOCK 17 END -->

<!-- BLOCK 18 START -->
## 18. Escoltas privados
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El escolta acompaña, defiende y protege a personas determinadas frente a agresiones o actos delictivos.

- El escolta privado protege a personas determinadas.
- Debe impedir agresiones o actos delictivos contra la persona protegida.
- El porte y uso de armas se somete a las condiciones legales del servicio.

:::visual
**Ilustración simple:** `t13-il-04-burbuja-proteccion.webp` · Escolta crea una burbuja móvil de protección alrededor de una persona.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El escolta acompaña, defiende y protege a personas determinadas frente a agresiones o actos delictivos.

La función es personal y preventiva. Su actuación y uso de armas quedan sujetos a habilitación, servicio autorizado y principios de necesidad y proporcionalidad.

### Claves normativas

- El escolta privado protege a personas determinadas.
- Debe impedir agresiones o actos delictivos contra la persona protegida.
- El porte y uso de armas se somete a las condiciones legales del servicio.

<!-- FACT:PN-T13-F052 -->
<!-- FACT:PN-T13-F053 -->
<!-- FACT:PN-T13-F054 -->

:::visual
**Ilustración simple:** `t13-il-04-burbuja-proteccion.webp` · Escolta crea una burbuja móvil de protección alrededor de una persona.
:::

:::hablemos-claro
El centro del servicio es la persona protegida.
:::

:::en-la-calle
La ruta, el entorno y la anticipación importan más que una reacción aparatosa.
:::

:::lo-que-cae
Escoltar no es investigar.
:::
<!-- ATESTADO END -->
<!-- BLOCK 18 END -->

<!-- BLOCK 19 START -->
## 19. Guardas rurales y especialidades
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Los guardas rurales protegen personas y bienes en fincas rústicas y desarrollan funciones en el medio rural.

- Los guardas rurales ejercen vigilancia y protección en fincas rústicas.
- Guardas de caza y guardapescas marítimos son especialidades del guarda rural.
- Deben colaborar y poner hechos delictivos a disposición de las Fuerzas y Cuerpos de Seguridad.
<!-- PARTE END -->
<!-- ATESTADO START -->
Los guardas rurales protegen personas y bienes en fincas rústicas y desarrollan funciones en el medio rural.

Sus especialidades amplían el ámbito: guardas de caza y guardapescas marítimos. La Guardia Civil ejerce competencias específicas de control.

### Claves normativas

- Los guardas rurales ejercen vigilancia y protección en fincas rústicas.
- Guardas de caza y guardapescas marítimos son especialidades del guarda rural.
- Deben colaborar y poner hechos delictivos a disposición de las Fuerzas y Cuerpos de Seguridad.

<!-- FACT:PN-T13-F055 -->
<!-- FACT:PN-T13-F056 -->
<!-- FACT:PN-T13-F057 -->

:::hablemos-claro
Rural es la categoría; caza y mar, sus especialidades.
:::

:::en-la-calle
Una actuación en finca debe preservar indicios y activar el cuerpo competente.
:::

:::lo-que-cae
Guardapescas marítimo sí forma parte del personal de seguridad privada.
:::
<!-- ATESTADO END -->
<!-- BLOCK 19 END -->

<!-- BLOCK 20 START -->
## 20. Jefes y directores de seguridad
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El jefe organiza servicios de la empresa; el director integra la seguridad de la entidad usuaria.

- El jefe de seguridad ejerce funciones de análisis, planificación y organización de servicios de la empresa.
- El director de seguridad organiza y administra recursos de seguridad de la entidad usuaria.
- Jefe y director cooperan con las Fuerzas y Cuerpos de Seguridad dentro de sus respectivos ámbitos.

:::visual
**Referencia visual prevista:** `t13-21-doble-puesto.webp` · Comparación jefe de seguridad y director de seguridad.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El jefe organiza servicios de la empresa; el director integra la seguridad de la entidad usuaria.

El jefe analiza riesgos, planifica y controla servicios y personal de la empresa. El director organiza recursos, propone sistemas y coordina la seguridad integral del usuario.

### Claves normativas

- El jefe de seguridad ejerce funciones de análisis, planificación y organización de servicios de la empresa.
- El director de seguridad organiza y administra recursos de seguridad de la entidad usuaria.
- Jefe y director cooperan con las Fuerzas y Cuerpos de Seguridad dentro de sus respectivos ámbitos.

<!-- FACT:PN-T13-F058 -->
<!-- FACT:PN-T13-F059 -->
<!-- FACT:PN-T13-F060 -->

:::visual
**Referencia visual prevista:** `t13-21-doble-puesto.webp` · Comparación jefe de seguridad y director de seguridad.
:::

:::hablemos-claro
Jefe mira desde la empresa prestadora; director, desde la organización protegida.
:::

:::en-la-calle
En un gran evento ambos pueden coincidir, pero no intercambian automáticamente sus responsabilidades.
:::

:::lo-que-cae
Dos puestos estratégicos, dos perspectivas.
:::
<!-- ATESTADO END -->
<!-- BLOCK 20 END -->

<!-- BLOCK 21 START -->
## 21. Detectives privados: funciones y límites
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El detective investiga hechos privados legítimos, delitos perseguibles a instancia de parte y vigilancia en determinados ámbitos.

- El detective investiga hechos y conductas privadas vinculados a un interés legítimo.
- Puede investigar delitos perseguibles a instancia de parte por encargo legitimado.
- Si conoce un delito perseguible de oficio debe denunciarlo inmediatamente y poner a disposición lo obtenido.

:::visual
**Ilustración simple:** `t13-il-05-investigacion-legitima.webp` · Detective sigue una ruta legítima y se detiene ante el límite de la intimidad.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El detective investiga hechos privados legítimos, delitos perseguibles a instancia de parte y vigilancia en determinados ámbitos.

No puede investigar delitos perseguibles de oficio; si conoce uno debe denunciarlo y entregar información e instrumentos relacionados.

### Claves normativas

- El detective investiga hechos y conductas privadas vinculados a un interés legítimo.
- Puede investigar delitos perseguibles a instancia de parte por encargo legitimado.
- Si conoce un delito perseguible de oficio debe denunciarlo inmediatamente y poner a disposición lo obtenido.

<!-- FACT:PN-T13-F061 -->
<!-- FACT:PN-T13-F062 -->
<!-- FACT:PN-T13-F063 -->

:::visual
**Ilustración simple:** `t13-il-05-investigacion-legitima.webp` · Detective sigue una ruta legítima y se detiene ante el límite de la intimidad.
:::

:::hablemos-claro
Interés legítimo abre la puerta; derechos fundamentales marcan las paredes.
:::

:::en-la-calle
Si aparece un delito público, cambia la prioridad: preservar y denunciar.
:::

:::lo-que-cae
Detective no es policía privada.
:::
<!-- ATESTADO END -->
<!-- BLOCK 21 END -->

<!-- BLOCK 22 START -->
## 22. Reglas comunes de prestación
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Todo servicio debe estar contratado, comunicado cuando proceda y prestarse por personal y empresa habilitados.

- No puede prestarse un servicio de seguridad privada sin contratación previa y, cuando proceda, autorización o comunicación.
- Los contratos deben formalizarse por escrito y comunicarse con la antelación reglamentaria.
- El servicio se presta por personal habilitado integrado en empresa autorizada cuando así lo exige la actividad.
<!-- PARTE END -->
<!-- ATESTADO START -->
Todo servicio debe estar contratado, comunicado cuando proceda y prestarse por personal y empresa habilitados.

La prestación se ajusta al contrato, instrucciones profesionales y marco público. Como regla, los vigilantes actúan dentro de inmuebles o propiedades, con excepciones legales.

### Claves normativas

- No puede prestarse un servicio de seguridad privada sin contratación previa y, cuando proceda, autorización o comunicación.
- Los contratos deben formalizarse por escrito y comunicarse con la antelación reglamentaria.
- El servicio se presta por personal habilitado integrado en empresa autorizada cuando así lo exige la actividad.

<!-- FACT:PN-T13-F064 -->
<!-- FACT:PN-T13-F065 -->
<!-- FACT:PN-T13-F066 -->

:::hablemos-claro
Contrato, comunicación, empresa, personal y lugar.
:::

:::en-la-calle
Antes de iniciar el turno, la orden de servicio debe coincidir con lo comunicado.
:::

:::lo-que-cae
Un servicio real no legaliza un contrato inexistente.
:::
<!-- ATESTADO END -->
<!-- BLOCK 22 END -->

<!-- BLOCK 23 START -->
## 23. Vigilancia, protección y armas
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Los servicios de vigilancia se prestan en los lugares y condiciones legales; el arma es excepcional y vinculada al servicio.

- La vigilancia puede extenderse fuera de inmuebles en los supuestos expresamente previstos.
- Los servicios con armas se limitan a los supuestos legal o reglamentariamente determinados.
- El arma reglamentaria se porta durante el servicio autorizado y se custodia conforme a la normativa.

:::visual
**Ilustración simple:** `t13-il-06-perimetro-servicio.webp` · Vigilante dentro de un perímetro claro con excepciones señaladas.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Los servicios de vigilancia se prestan en los lugares y condiciones legales; el arma es excepcional y vinculada al servicio.

Existen supuestos en que la vigilancia puede extenderse a vías o espacios de uso común. Los servicios armados requieren determinación legal o administrativa y arma reglamentaria.

### Claves normativas

- La vigilancia puede extenderse fuera de inmuebles en los supuestos expresamente previstos.
- Los servicios con armas se limitan a los supuestos legal o reglamentariamente determinados.
- El arma reglamentaria se porta durante el servicio autorizado y se custodia conforme a la normativa.

<!-- FACT:PN-T13-F067 -->
<!-- FACT:PN-T13-F068 -->
<!-- FACT:PN-T13-F069 -->

:::visual
**Ilustración simple:** `t13-il-06-perimetro-servicio.webp` · Vigilante dentro de un perímetro claro con excepciones señaladas.
:::

:::hablemos-claro
El arma acompaña a un servicio habilitado; no acompaña a la persona por costumbre.
:::

:::en-la-calle
Delimita físicamente el servicio para no invadir el espacio público.
:::

:::lo-que-cae
Excepción no es permiso general.
:::
<!-- ATESTADO END -->
<!-- BLOCK 23 END -->

<!-- BLOCK 24 START -->
## 24. Videovigilancia
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La videovigilancia capta, graba, trata y almacena imágenes para prevenir y proteger dentro de límites de proporcionalidad y privacidad.

- La videovigilancia de seguridad debe respetar proporcionalidad y normativa de protección de datos.
- Las cámaras que forman parte de medidas obligatorias o sistemas de alarma no requieren autorización administrativa individual para su instalación o uso en los términos del artículo 42.
- La visualización profesional de sistemas de seguridad corresponde al personal y servicios habilitados cuando la ley lo reserva.

:::visual
**Referencia visual prevista:** `t13-25-cono-vision.webp` · Conos de visión permitidos y zonas de privacidad protegida.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
La videovigilancia capta, graba, trata y almacena imágenes para prevenir y proteger dentro de límites de proporcionalidad y privacidad.

Las cámaras conectadas a centrales de alarma o integradas en medidas obligatorias siguen reglas específicas. No toda cámara doméstica convierte a su titular en empresa de seguridad.

### Claves normativas

- La videovigilancia de seguridad debe respetar proporcionalidad y normativa de protección de datos.
- Las cámaras que forman parte de medidas obligatorias o sistemas de alarma no requieren autorización administrativa individual para su instalación o uso en los términos del artículo 42.
- La visualización profesional de sistemas de seguridad corresponde al personal y servicios habilitados cuando la ley lo reserva.

<!-- FACT:PN-T13-F070 -->
<!-- FACT:PN-T13-F071 -->
<!-- FACT:PN-T13-F072 -->

:::visual
**Referencia visual prevista:** `t13-25-cono-vision.webp` · Conos de visión permitidos y zonas de privacidad protegida.
:::

:::hablemos-claro
Finalidad, campo de visión, acceso y conservación.
:::

:::en-la-calle
Una cámara bien instalada puede ser ilegal si apunta donde no debe o conserva sin control.
:::

:::lo-que-cae
Videovigilar no es grabarlo todo.
:::
<!-- ATESTADO END -->
<!-- BLOCK 24 END -->

<!-- BLOCK 25 START -->
## 25. Protección personal, depósito y transporte
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
La ley diferencia protección de personas, custodia de bienes valiosos y transporte de seguridad.

- La protección personal se presta por escoltas privados en servicios autorizados.
- El depósito de objetos valiosos exige instalaciones y medidas de seguridad adecuadas.
- El transporte de fondos y valores mantiene una cadena de custodia con medios y personal reglamentarios.
<!-- PARTE END -->
<!-- ATESTADO START -->
La ley diferencia protección de personas, custodia de bienes valiosos y transporte de seguridad.

Cada servicio requiere personal, vehículos, instalaciones, comunicación y medidas adecuados al riesgo. El depósito y el transporte forman una cadena de custodia.

### Claves normativas

- La protección personal se presta por escoltas privados en servicios autorizados.
- El depósito de objetos valiosos exige instalaciones y medidas de seguridad adecuadas.
- El transporte de fondos y valores mantiene una cadena de custodia con medios y personal reglamentarios.

<!-- FACT:PN-T13-F073 -->
<!-- FACT:PN-T13-F074 -->
<!-- FACT:PN-T13-F075 -->

:::hablemos-claro
Persona, depósito y ruta son riesgos distintos.
:::

:::en-la-calle
En transporte, cada transferencia debe dejar claro quién recibe, qué recibe y cuándo.
:::

:::lo-que-cae
Custodiar no es simplemente guardar.
:::
<!-- ATESTADO END -->
<!-- BLOCK 25 END -->

<!-- BLOCK 26 START -->
## 26. Instalación, mantenimiento y alarmas
**Fuente principal:** `L5-2014-T13,OINT316-2011-T13`
<!-- PARTE START -->
Los sistemas conectados a centrales y la gestión de alarmas son actividades reservadas sometidas a verificación y comunicación.

- La instalación de sistemas conectados a centrales receptoras es actividad de seguridad privada.
- La central debe verificar las señales por los procedimientos reglamentarios antes de comunicar cuando así proceda.
- La comunicación a las Fuerzas y Cuerpos de Seguridad debe aportar datos útiles sobre la alarma.
<!-- PARTE END -->
<!-- ATESTADO START -->
Los sistemas conectados a centrales y la gestión de alarmas son actividades reservadas sometidas a verificación y comunicación.

La central recibe señales, aplica procedimientos de verificación y comunica alarmas reales a las Fuerzas y Cuerpos de Seguridad. Las falsas alarmas reiteradas tienen consecuencias.

### Claves normativas

- La instalación de sistemas conectados a centrales receptoras es actividad de seguridad privada.
- La central debe verificar las señales por los procedimientos reglamentarios antes de comunicar cuando así proceda.
- La comunicación a las Fuerzas y Cuerpos de Seguridad debe aportar datos útiles sobre la alarma.

<!-- FACT:PN-T13-F076 -->
<!-- FACT:PN-T13-F077 -->
<!-- FACT:PN-T13-F078 -->

:::hablemos-claro
Señal, verificación, clasificación y comunicación.
:::

:::en-la-calle
Una buena verificación reduce falsas movilizaciones sin retrasar una alarma real.
:::

:::lo-que-cae
Central receptora no es central de castigo.
:::
<!-- ATESTADO END -->
<!-- BLOCK 26 END -->

<!-- BLOCK 27 START -->
## 27. Investigación privada e informes
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
El informe documenta hechos investigados, medios y resultados dentro del encargo legítimo.

- El informe de investigación se entrega al cliente legitimado y documenta el resultado del encargo.
- Los informes y soportes se conservan durante el plazo legal con medidas de reserva.
- La investigación no puede utilizar medios que vulneren honor, intimidad, imagen o secreto de comunicaciones.

:::visual
**Ilustración simple:** `t13-il-07-informe-bajo-llave.webp` · Informe de detective guardado bajo llave y entregado al destinatario legítimo.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
El informe documenta hechos investigados, medios y resultados dentro del encargo legítimo.

Debe conservarse con reserva y estar disponible para autoridades competentes en los supuestos legales. El detective no puede utilizar medios que lesionen derechos fundamentales.

### Claves normativas

- El informe de investigación se entrega al cliente legitimado y documenta el resultado del encargo.
- Los informes y soportes se conservan durante el plazo legal con medidas de reserva.
- La investigación no puede utilizar medios que vulneren honor, intimidad, imagen o secreto de comunicaciones.

<!-- FACT:PN-T13-F079 -->
<!-- FACT:PN-T13-F080 -->
<!-- FACT:PN-T13-F081 -->

:::visual
**Ilustración simple:** `t13-il-07-informe-bajo-llave.webp` · Informe de detective guardado bajo llave y entregado al destinatario legítimo.
:::

:::hablemos-claro
Informe útil, fuente lícita y destinatario legitimado.
:::

:::en-la-calle
Escribe lo observado y acreditado, no lo que el cliente esperaba encontrar.
:::

:::lo-que-cae
Reserva no permite fabricar pruebas.
:::
<!-- ATESTADO END -->
<!-- BLOCK 27 END -->

<!-- BLOCK 28 START -->
## 28. Adopción y tipos de medidas de seguridad
**Fuente principal:** `L5-2014-T13,OINT317-2011-T13`
<!-- PARTE START -->
Las medidas pueden imponerse por norma o resolución y combinar componentes físicos, electrónicos, informáticos, organizativos y personales.

- Las medidas de seguridad pueden ser físicas, electrónicas, informáticas, organizativas o personales.
- Determinados establecimientos están obligados a adoptar medidas por su riesgo.
- Los elementos deben cumplir homologación, certificación o verificación cuando sea preceptivo.

:::visual
**Referencia visual prevista:** `t13-29-capas-medidas.webp` · Edificio protegido por capas físicas, electrónicas, organizativas y personales.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Las medidas pueden imponerse por norma o resolución y combinar componentes físicos, electrónicos, informáticos, organizativos y personales.

La respuesta debe ser proporcional al riesgo y cumplir homologación o certificación cuando proceda. Una sola medida rara vez sustituye a un sistema coherente.

### Claves normativas

- Las medidas de seguridad pueden ser físicas, electrónicas, informáticas, organizativas o personales.
- Determinados establecimientos están obligados a adoptar medidas por su riesgo.
- Los elementos deben cumplir homologación, certificación o verificación cuando sea preceptivo.

<!-- FACT:PN-T13-F082 -->
<!-- FACT:PN-T13-F083 -->
<!-- FACT:PN-T13-F084 -->

:::visual
**Referencia visual prevista:** `t13-29-capas-medidas.webp` · Edificio protegido por capas físicas, electrónicas, organizativas y personales.
:::

:::hablemos-claro
Capas distintas contra riesgos distintos.
:::

:::en-la-calle
Una caja fuerte sin procedimiento de apertura ni alarma forma una defensa incompleta.
:::

:::lo-que-cae
Homologado no significa eterno.
:::
<!-- ATESTADO END -->
<!-- BLOCK 28 END -->

<!-- BLOCK 29 START -->
## 29. Establecimientos obligados y medidas específicas
**Fuente principal:** `OINT317-2011-T13`
<!-- PARTE START -->
Entidades de crédito, joyerías, estaciones de servicio, farmacias, administraciones de lotería y otros establecimientos tienen reglas diferenciadas.

- Las entidades de crédito deben adoptar las medidas específicas previstas para su actividad.
- Joyerías y establecimientos de especial riesgo tienen obligaciones adaptadas a valores y exposición.
- Las administraciones de lotería están entre los establecimientos con medidas específicas, pero cada obligación debe comprobarse en la norma vigente.
<!-- PARTE END -->
<!-- ATESTADO START -->
Entidades de crédito, joyerías, estaciones de servicio, farmacias, administraciones de lotería y otros establecimientos tienen reglas diferenciadas.

Las medidas dependen del tipo de establecimiento, valores manejados y condiciones reglamentarias. Memorizar una medida sin identificar su establecimiento genera errores.

### Claves normativas

- Las entidades de crédito deben adoptar las medidas específicas previstas para su actividad.
- Joyerías y establecimientos de especial riesgo tienen obligaciones adaptadas a valores y exposición.
- Las administraciones de lotería están entre los establecimientos con medidas específicas, pero cada obligación debe comprobarse en la norma vigente.

<!-- FACT:PN-T13-F085 -->
<!-- FACT:PN-T13-F086 -->
<!-- FACT:PN-T13-F087 -->

:::hablemos-claro
Primero identifica establecimiento; después busca su paquete de medidas.
:::

:::en-la-calle
Una respuesta antigua puede haber sido anulada o modificada: no la reutilices sin cotejo.
:::

:::lo-que-cae
No traslades la medida de un banco a una farmacia.
:::
<!-- ATESTADO END -->
<!-- BLOCK 29 END -->

<!-- BLOCK 30 START -->
## 30. Inspección y deber de colaboración
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Las Fuerzas y Cuerpos de Seguridad inspeccionan entidades, servicios, personal y medidas dentro de su competencia.

- La inspección puede abarcar empresas, despachos, servicios, personal, centros y establecimientos obligados.
- Los inspeccionados deben facilitar información y acceso a documentación exigible.
- El acta de inspección documenta hechos sin sustituir por sí sola la resolución sancionadora.

:::visual
**Ilustración simple:** `t13-il-08-inspeccion-en-capas.webp` · Equipo inspector revisa documentos, personal, sistema y establecimiento.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Las Fuerzas y Cuerpos de Seguridad inspeccionan entidades, servicios, personal y medidas dentro de su competencia.

Los sujetos inspeccionados deben facilitar acceso, información y documentación. Las actas recogen hechos constatados y pueden activar corrección o sanción.

### Claves normativas

- La inspección puede abarcar empresas, despachos, servicios, personal, centros y establecimientos obligados.
- Los inspeccionados deben facilitar información y acceso a documentación exigible.
- El acta de inspección documenta hechos sin sustituir por sí sola la resolución sancionadora.

<!-- FACT:PN-T13-F088 -->
<!-- FACT:PN-T13-F089 -->
<!-- FACT:PN-T13-F090 -->

:::visual
**Ilustración simple:** `t13-il-08-inspeccion-en-capas.webp` · Equipo inspector revisa documentos, personal, sistema y establecimiento.
:::

:::hablemos-claro
Inspeccionar es comprobar; sancionar exige su procedimiento.
:::

:::en-la-calle
Ten disponible autorización, contratos, comunicaciones, personal y mantenimiento.
:::

:::lo-que-cae
Acta y resolución no son el mismo documento.
:::
<!-- ATESTADO END -->
<!-- BLOCK 30 END -->

<!-- BLOCK 31 START -->
## 31. Control, corrección y repaso operativo
**Fuente principal:** `L5-2014-T13`
<!-- PARTE START -->
Resolver bien exige identificar sujeto, actividad, autorización, personal, función, servicio, medida y órgano de control.

- La calificación correcta comienza por identificar quién actúa y qué actividad o función realiza.
- Después se comprueban habilitación, autorización, contrato, lugar y límites de la actuación.
- La cooperación policial no transforma al personal privado en funcionario, salvo la protección jurídica concreta prevista.

:::visual
**Referencia visual prevista:** `t13-32-tablero-ocho-casillas.webp` · Tablero de ocho casillas para resolver supuestos de seguridad privada.
:::
<!-- PARTE END -->
<!-- ATESTADO START -->
Resolver bien exige identificar sujeto, actividad, autorización, personal, función, servicio, medida y órgano de control.

Las preguntas históricas explotan parejas cercanas: actividad/servicio, jefe/director, vigilante/escolta, empresa/despacho y seguridad pública/privada.

### Claves normativas

- La calificación correcta comienza por identificar quién actúa y qué actividad o función realiza.
- Después se comprueban habilitación, autorización, contrato, lugar y límites de la actuación.
- La cooperación policial no transforma al personal privado en funcionario, salvo la protección jurídica concreta prevista.

<!-- FACT:PN-T13-F091 -->
<!-- FACT:PN-T13-F092 -->
<!-- FACT:PN-T13-F093 -->

:::visual
**Referencia visual prevista:** `t13-32-tablero-ocho-casillas.webp` · Tablero de ocho casillas para resolver supuestos de seguridad privada.
:::

:::hablemos-claro
Ocho casillas: sujeto, actividad, empresa, personal, función, servicio, medida y control.
:::

:::en-la-calle
Si una opción mezcla dos categorías, probablemente está construida como distractor.
:::

:::lo-que-cae
La precisión vence a la intuición.
:::
<!-- ATESTADO END -->
<!-- BLOCK 31 END -->

<!-- LAYER:MAPA -->
# Mapa del tema

:::visual
**Mapa general previsto:** `t13-01-mapa-general.webp` · recorrido visual por las seis partes y sus relaciones.
:::

El Tema 13 se estudia como un sistema de decisiones enlazadas, no como una colección de listas aisladas.

<!-- LAYER:CONTENIDO -->
# Contenido

El contenido se desarrolla en 31 bloques semánticos sincronizados con El Parte y El Atestado.

<!-- LAYER:HABLEMOS_CLARO -->
# Hablemos claro

:::hablemos-claro
Lee primero el sujeto, después la figura jurídica y por último la competencia, el procedimiento o el efecto.
:::

<!-- LAYER:EN_LA_CALLE -->
# En la calle

:::en-la-calle
Una actuación correcta fija hechos, activa garantías y documenta el cauce empleado antes de llegar a la conclusión.
:::

<!-- LAYER:LO_QUE_CAE -->
# Lo que cae

:::lo-que-cae
Prioriza definiciones próximas, sujetos competentes, secuencias, límites, excepciones y diferencias entre figuras vecinas.
:::

<!-- LAYER:HA_CAIDO -->
# Ha caído

:::ha-caido
Las coincidencias históricas se conservan en el índice interno y permanecen ocultas mientras no exista plantilla oficial final verificada.
:::
