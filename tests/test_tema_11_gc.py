...
K = ROOT / "conocimiento/guardia-civil/tema-11"
B = ROOT / "banco-preguntas/guardia-civil/tema-11"
A = ROOT / "assets/guardia-civil/tema-11"

class Tema11GuardiaCivilQualityGate(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_topic_contract(self):
        self.assertEqual(42, len(self.coverage["blocks"]))
        self.assertEqual(self.coverage["total_atomic_facts"], len(self.coverage["facts"]))
        self.assertIn("artículo 53 bis", self.master.lower())
        self.assertIn("28/12/2025", self.master)
    ...
    def test_official_mapping(self):
        self.assertEqual(36, self.index["total_referencias"])
        self.assertEqual(36, self.index["con_respuesta_verificada"])
        items = {q["question_id"]: q for q in self.index["questions"]}
        self.assertIn("GC-T11", items["of-gc-p2023-1a-q066"]["fact_refs"][0])
        facts = {f["id"]: f for f in self.coverage["facts"]}
        self.assertEqual(42, facts[items["of-gc-p2023-1a-q066"]["fact_refs"][0]]["bloque"])
    ...
    def test_visuals_only_planned(self):
        ...
        self.assertEqual(43, manifest["totals"]["planned"])
