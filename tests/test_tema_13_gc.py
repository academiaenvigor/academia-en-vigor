...
K = ROOT / "conocimiento/guardia-civil/tema-13"
B = ROOT / "banco-preguntas/guardia-civil/tema-13"
A = ROOT / "assets/guardia-civil/tema-13"

class Tema13GuardiaCivilQualityGate(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_topic_contract(self):
        self.assertEqual(57, len(self.coverage["blocks"]))
        self.assertEqual(self.coverage["total_atomic_facts"], len(self.coverage["facts"]))
        self.assertIn("artículo 54", self.master.lower())
        self.assertIn("artículo 55", self.master.lower())
        self.assertNotIn("artículo 56.", self.master.lower())
    ...
    def test_official_mapping(self):
        self.assertEqual(65, self.index["total_referencias"])
        self.assertEqual(65, self.index["con_respuesta_verificada"])
        items = {q["question_id"]: q for q in self.index["questions"]}
        self.assertIn("GC-T13", items["of-gc-p2024-3a-q064"]["fact_refs"][0])
